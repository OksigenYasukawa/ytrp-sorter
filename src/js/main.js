...
function init() {
  ...
  /**for (let i = 0; i <=100; i++) {
    ...
    select.value = i;
    select.text = i;
    if (i === 100) { select.selected = 'selected'; }
...
function result(imageNum = 100) {
  ...
  const imgRes = (char, num) => {
    ...
    return `<div class="result image"><div class="left"></div><div class="right"><img src="${char.img}"><span class="nomor">${num}</span><div><span title="${charTooltip}">${charName}</span></div></div></div>`;
  }
  const res = (char, num) => {
    const charName = reduceTextWidth(char.name, 'Arial 12px', 160);
    const charTooltip = char.name !== charName ? char.name : '';
